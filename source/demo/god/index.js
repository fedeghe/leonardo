window.onload = function () {
    var target = document.getElementById('trg'),
        width = 987,
        height = 652,
        w = function (p) {return width * p/100;},
        h = function (p) {return height * p/100;},
        Leo = Leonardo(width, height, { ns: '*', target: target }),
        img = Leo.image(0,0,width, height, './god.jpg').setAttributes({opacity: 0.6}),
        main = Leo.group(),
        fillStyle = {
			"stroke-width": 2,
			"stroke": '#ef88d8',
			"stroke-opacity": 1,
			"fill-opacity": 10,
			"stroke-linejoin": "round",
			fill: 'transparent'
		},
        builder = function (acc, e) {
            return acc.l(w(e[0]), h(e[1]))
        },


        leftmid = Leo.path([
                [0.3, 3.07],
                [0.81, 3.07],
                [-0.1, 2.91],
                [0.91, 3.53],
                [-1.11, 0.46],
                [-1.11, -0.92],
                [-1.01, -2.3],
                [-1.11, -3.68],
                [-0.81, -1.99],
                [-0.41, -3.99]
            ].reduce(builder, Leo.pathBuild.M(w(44.07), h(50.61)))
        ).setAttributes(fillStyle),

        leftmid2 = Leo.path([
                [0.51, 2.76],
                [-1.11, 0],
                [-1.11, -0.92],
                [-0.3, -1.99],
                [0, -1.99],
                [-0.51, -0.46],
                [-0.71, -1.69],
                [-0.2, -1.99],
                [-0.71, -0.77],
                [-0.91, -0.77],
                [-1.11, -2.3],
                [0.3, 2.45],
                [-0.1, 2.61]
            ].reduce(builder, Leo.pathBuild.M(w(42.86), h(60.89)))
        ).setAttributes(fillStyle),

        leftLast = Leo.path([
                [-0.71, 2.15],
                [-0.61, 0.31],
                [-0.91, -1.23],
                [-0.1, -1.53],
                [0.2, -1.99]
            ].reduce(builder, Leo.pathBuild.M(w(34.95), h(61.96)))
        ).setAttributes(fillStyle),

        // leftHole = Leo.path(Leo.pathBuild.M(w(33.33), h(51.84))
        // .l(w(-0.2), h(-1.99))
        // .l(w(-0.61), h(-1.38))
        // .l(w(-1.42), h(0.31))).setAttributes(fillStyle),

        leftHole = Leo.path([
                [-0.2, -1.99],
                [-0.61, -1.38],
                [-1.42, 0.31]
            ].reduce(builder, Leo.pathBuild.M(w(33.33), h(51.84)))
        ).setAttributes(fillStyle),

        left1 = Leo.path([
                [4.86, -1.69],
                [3.44, -3.37],
                [3.24, -1.69],
                [3.04, -0.61],
                [4.15, 0.61],
                [4.26, 1.38],
                [3.65, 1.07],
                [4.26, 0.31],
                [2.84, 0.77],
                [1.52, 2.15],
                [1.32, 2.45],
                [3.44, 1.07],
                [1.72, 1.69],
                [1.62, 0.77],
                [1.22, 2.61],
                [1.93, 3.83],
                [0.91, 2.91],
                [1.93, 3.37],
                [-1.11, 1.23],
                [-1.52, -0.15],
                [-1.52, -1.53],
                [-0.61, -1.99],
                [-1.62, -1.84],
                [-1.82, -3.37],
                [-1.25, -0.20],
                [-2.94, -0.92],
                [-0.81, -0.92],
                [-1.32, 1.07],
                [-2.33, -0.46],
                [-0.41, -0.92],
                [-2.23, 1.69],
                [-0.71, 0.92],
                [2.43, 4.29],
                [1.52, 0.77],
                [1.62, 1.99],
                [1.42, 2.3],
                [0.91, 3.37],
                [-0.2, 1.84],
                [-1.42, 0.92],
                [-1.42, -0.92],
                [-2.13, -2.3],
                [-1.32, -1.99],
                [-1.93, -1.99],
                [-2.33, -1.53],
                [-2.43, -1.07],
                [-2.03, -0.15],
                [-1.72, -1.07],
                [-2.03, -2.15],
                [-1.52, -3.22],
                [-1.22, 1.07],
                [-1.93, 0.77],
                [-1.82, 1.69],
                [-1.72, 3.07],
                [-1.72, 2.91],
                [-1.93, 2.91],
                [-1.93, 2.45],
                [-1.93, 2.15],
                [-2.53, 2.15]
            ].reduce(builder, Leo.pathBuild.M(w(1.11), h(37.12)))
        ).setAttributes(fillStyle),
        

        right1 = Leo.path([
                [-1.42, -0.15],
                [-1.82, 0.77],
                [-1.62, -0.15],
                [-1.32, 0.15],
                [-1.52, 0.61],
                [-1.22, 1.07],
                [-1.22, 0.77],
                [-1.42, 0],
                [-1.72, 0],
                [-1.62, 1.07],
                [-1.62, 1.23],
                [-1.52, 0],
                [-1.72, 0],
                [-1.82, 0.61],
                [-1.42, 0.77],
                [-1.72, 0.77],
                [-1.62, 1.38],
                [-1.32, 1.07],
                [-1.22, 0.61],
                [-1.11, 0.46],
                [-1.01, 0.15],
                [-1.32, 0],
                [-1.01, 0.61],
                [-1.52, 0.31],
                [-1.52, -0.15],
                [-1.22, -0.15],
                [-0.91, 0.31],
                [-1.22, -0.15],
                [-1.52, 0.92],
                [-1.11, 0.61],
                [-0.91, 0.46],
                [-0.81, 0],
                [-0.61, 0.46],
                [-1.32, 0.92],
                [-0.91, 0.46],
                [-0.41, 1.07],
                [0.2, 1.38],
                [0.81, 0.61],
                [1.01, 0.46],
                [1.52, 0],
                [1.22, -0.46],
                [0.81, -0.61],
                [1.32, 0.15],
                [1.42, -0.46],
                [1.01, -0.46],
                [1.11, 0.61],
                [1.72, 0.31],
                [1.11, 0.15],
                [0.71, 0],
                [0.51, 0.92],
                [1.11, 0.92],
                [-0.61, 1.23],
                [-0.41, 1.07],
            ].reduce(builder, Leo.pathBuild.M(w(98.99), h(34.66)))
        ).setAttributes(fillStyle),

        right2 = Leo.path([
                [-1.42, 1.99],
                [-1.11, 2.61],
                [-1.52, 1.53],
                [-1.22, 0.46],
                [-0.3, 0.92],
                [-0.81, -0.15],
                [-1.22, 0.77],
                [-0.61, 1.23],
                [-1.93, 1.38],
                [-0.61, 1.69],
                [0.3, 1.53],
                [0.81, 1.07],
                [1.22, 0.77],
                [2.13, -0.15],
                [1.72, -1.07],
                [1.32, -1.99],
                [1.52, -1.38],
                [1.72, -1.23],
                [1.62, -0.92],
                [1.42, 0.46],
                [1.52, 0.15],
                [1.93, -0.46],
                [1.72, -0.61],
                [1.11, -1.23],
                [1.32, -1.99],
            ].reduce(builder, Leo.pathBuild.M(w(73.35), h(49.54)))
        ).setAttributes(fillStyle),
        
        right3 = Leo.path([
                [0, 2.76],
                [-0.41, 1.84],
                [0, 1.69],
                [0.3, 1.53],
                [0.1, 3.07],
                [0.91, 1.38],
                [1.42, -0.15],
                [0.41, -1.23],
                [0.2, -2.3],
                [-0.2, -2.15],
                [0.3, -1.53],
                [0.1, -1.99],
                [0.3, -1.99],
                [1.01, -2.3],
            ].reduce(builder, Leo.pathBuild.M(w(64.54), h(65.18)))
        ).setAttributes(fillStyle),

        right4 = Leo.path([
                [0.71, 0.77],
                [1.42, -0.46],
                [0.51, -1.69],
                [0.3, -1.84],
                [-0.3, -1.23],
                [0.71, -1.53],
                [0.41, -1.84],
                [0.1, -1.23],
                [1.22, -1.38],
                [1.01, -1.99],
                [-0.3, 2.61],
                [-0.51, 1.84],
                [-0.1, 1.69],
                [0, 1.38],
                [0.3, 2.15],
                [0.61, 1.69],
                [1.32, 0.31],
                [0.81, -0.92],
                [0.41, -1.99],
                [-0.1, -1.53],
                [0, -1.99],
                [0.51, -1.23],
                [0.41, -1.23],
                [0, -1.07],
                [1.93, -0.15],
                [1.11, 0],
                [1.01, -0.92],
                [1.32, -1.23],
                [1.22, -0.61],
                [0.81, -0.92],
                [0.81, -1.69],
                [1.32, -1.07],
                [0.71, -1.07],
                [0.71, -0.61],
                [0.2, -1.07],
                [0.81, 0.15],
                [0.61, -0.61],
                [1.52, 0],
                [1.32, 0.31],
                [1.42, 0.46],
                [1.32, 0.61],
                [1.22, 0.46],
                [1.72, 0.31],
                [1.22, 0.31]
            ].reduce(builder, Leo.pathBuild.M(w(67.17), h(76.53)))
        ).setAttributes(fillStyle);


    main.append(img)

    Leo.append(main);
    Leo.append(left1, leftmid, leftmid2, leftLast, leftHole);
    Leo.append(right1, right2, right3, right4);
    
    Leo.render();
    Leo.positionInspector('[{RPx}, {RPy}],');
    // Leo.positionInspector();
    document.body.appendChild(Leo.downloadAnchor())
}
