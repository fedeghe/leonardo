(function() {
    ...
    var days = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'],
        qs = Leonardo.getqs(),
        state = {
            theme: ('theme' in qs && qs.theme.match(/white|black/)) ? qs.theme : 'white',
            dayVisible: 'day' in qs,
            dateVisible: 'date' in qs,
            handlers: []
        };
    function freeHandlers(){
        state.handlers.forEach(function (h){h()})
        state.handlers = []
    }
    function getDate() {return new Date().getDate().toString();}
    function getDay() {return days[new Date().getDay()];}
    function getL(target, size){
        ...
        var daydate = [{
            condition: state.dateVisible,
            getText: getDate,
            position: [cx * 1.4, cy * .945], 
            size: [size/14, size/17] 
        },{
            condition: state.dayVisible,
            getText: getDay,
    ...
    function render() {
        ...
        L.render({
            ...
            cb: function() {
                ...
                state.handlers = state.handlers.concat([{el: withDay, s: 'dayVisible'}, {el: withDate, s: 'dateVisible'}].map(function(i) {
                    function _(e) {
                        // var u = search.get()
                    ...
                    i.el.addEventListener('click', _)    
                    return function () {i.el.removeEventListener('click', _)};
                }));

                state.handlers = state.handlers.concat([{btn: whiteButton, theme: 'white'}, {btn: blackButton, theme: 'black'}].map(function(i) {
                    function _(e) {
                        // var u = search.get()
                        // u.set('theme', i.theme)
                        // search.commit(u)
                    ...
                    i.btn.addEventListener('click', _)    
                    return function (){i.btn.removeEventListener('click', _)};
                }));
